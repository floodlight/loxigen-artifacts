...
void
of_bsn_tlv_disable_xmit_push_wire_types(of_object_t *obj)
{
    unsigned char *buf = OF_OBJECT_BUFFER_INDEX(obj, 0);
    switch (obj->version) {
    case OF_VERSION_1_3:
...
/**
 * \defgroup of_bsn_tlv_disable_xmit of_bsn_tlv_disable_xmit
 */

/**
 * Create a new of_bsn_tlv_disable_xmit object
 *
 * @param version The wire version to use for the object
 * @return Pointer to the newly create object or NULL on error
 *
 ...
 * a new underlying wire buffer.
 *
 * \ingroup of_bsn_tlv_disable_xmit
 */
...
of_bsn_tlv_disable_xmit_new(of_version_t version)
{
    ...
    bytes = of_object_fixed_len[version][OF_BSN_TLV_DISABLE_XMIT];

    if ((obj = of_object_new(bytes)) == NULL) {
        return NULL;
    }

    of_bsn_tlv_disable_xmit_init(obj, version, bytes, 0);
    of_bsn_tlv_disable_xmit_push_wire_types(obj);
    of_tlv16_wire_length_set(obj, obj->length);
...
/**
 * Initialize an object of type of_bsn_tlv_disable_xmit.
 *
 * @param obj Pointer to the object to initialize
 * @param version The wire version to use for the object
 * @param bytes How many bytes in the object
...
void
of_bsn_tlv_disable_xmit_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_BSN_TLV_DISABLE_XMIT] >= 0);
    if (clean_wire) {
        MEMSET(obj, 0, sizeof(*obj));
    }
    if (bytes < 0) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_BSN_TLV_DISABLE_XMIT;
...
void
of_bsn_tlv_dns_analytics_push_wire_types(of_object_t *obj)
{
    unsigned char *buf = OF_OBJECT_BUFFER_INDEX(obj, 0);
    switch (obj->version) {
    case OF_VERSION_1_3:
    case OF_VERSION_1_4:
        *(uint16_t *)(buf + 0) = U16_HTON(0xbe); /* type */
        break;
    default:
        UNREACHABLE();
    }
...
/**
 * \defgroup of_bsn_tlv_dns_analytics of_bsn_tlv_dns_analytics
 */

/**
 * Create a new of_bsn_tlv_dns_analytics object
 *
 * @param version The wire version to use for the object
 * @return Pointer to the newly create object or NULL on error
 *
...
of_bsn_tlv_dns_analytics_new(of_version_t version)
{
    ...
    bytes = of_object_fixed_len[version][OF_BSN_TLV_DNS_ANALYTICS];

    if ((obj = of_object_new(bytes)) == NULL) {
        return NULL;
    }

    of_bsn_tlv_dns_analytics_init(obj, version, bytes, 0);
    of_bsn_tlv_dns_analytics_push_wire_types(obj);
    of_tlv16_wire_length_set(obj, obj->length);
...
/**
 * Initialize an object of type of_bsn_tlv_dns_analytics.
 *
 * @param obj Pointer to the object to initialize
 * @param version The wire version to use for the object
 * @param bytes How many bytes in the object
...
void
of_bsn_tlv_dns_analytics_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_BSN_TLV_DNS_ANALYTICS] >= 0);
    if (clean_wire) {
        MEMSET(obj, 0, sizeof(*obj));
    }
    if (bytes < 0) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_BSN_TLV_DNS_ANALYTICS;
...
void
of_bsn_tlv_drop_control_push_wire_types(of_object_t *obj)
{
    unsigned char *buf = OF_OBJECT_BUFFER_INDEX(obj, 0);
    switch (obj->version) {
    case OF_VERSION_1_3:
...
/**
 * \defgroup of_bsn_tlv_drop_control of_bsn_tlv_drop_control
 */

/**
 * Create a new of_bsn_tlv_drop_control object
 *
 * @param version The wire version to use for the object
 * @return Pointer to the newly create object or NULL on error
 *
 ...
 * a new underlying wire buffer.
 *
 * \ingroup of_bsn_tlv_drop_control
 */

of_object_t *
of_bsn_tlv_drop_control_new(of_version_t version)
{
    of_object_t *obj;
    int bytes;

    bytes = of_object_fixed_len[version][OF_BSN_TLV_DROP_CONTROL];

    if ((obj = of_object_new(bytes)) == NULL) {
        return NULL;
    }

    of_bsn_tlv_drop_control_init(obj, version, bytes, 0);
    of_bsn_tlv_drop_control_push_wire_types(obj);
    of_tlv16_wire_length_set(obj, obj->length);
...
/**
 * Initialize an object of type of_bsn_tlv_drop_control.
 *
 * @param obj Pointer to the object to initialize
 * @param version The wire version to use for the object
 * @param bytes How many bytes in the object
...
void
of_bsn_tlv_drop_control_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_BSN_TLV_DROP_CONTROL] >= 0);
    if (clean_wire) {
        MEMSET(obj, 0, sizeof(*obj));
    }
    if (bytes < 0) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_BSN_TLV_DROP_CONTROL;
...
void
of_bsn_tlv_egress_sampling_rate_push_wire_types(of_object_t *obj)
{
    unsigned char *buf = OF_OBJECT_BUFFER_INDEX(obj, 0);
    switch (obj->version) {
    case OF_VERSION_1_3:
    case OF_VERSION_1_4:
        *(uint16_t *)(buf + 0) = U16_HTON(0xc3); /* type */
        break;
    default:
        UNREACHABLE();
    }
...
/**
 * \defgroup of_bsn_tlv_egress_sampling_rate of_bsn_tlv_egress_sampling_rate
 */

/**
 * Create a new of_bsn_tlv_egress_sampling_rate object
 *
 * @param version The wire version to use for the object
 * @return Pointer to the newly create object or NULL on error
 *
...
of_bsn_tlv_egress_sampling_rate_new(of_version_t version)
{
    ...
    bytes = of_object_fixed_len[version][OF_BSN_TLV_EGRESS_SAMPLING_RATE];

    if ((obj = of_object_new(bytes)) == NULL) {
        return NULL;
    }

    of_bsn_tlv_egress_sampling_rate_init(obj, version, bytes, 0);
    of_bsn_tlv_egress_sampling_rate_push_wire_types(obj);
    of_tlv16_wire_length_set(obj, obj->length);
...
/**
 * Initialize an object of type of_bsn_tlv_egress_sampling_rate.
 *
 * @param obj Pointer to the object to initialize
 * @param version The wire version to use for the object
 * @param bytes How many bytes in the object
...
void
of_bsn_tlv_egress_sampling_rate_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_BSN_TLV_EGRESS_SAMPLING_RATE] >= 0);
    if (clean_wire) {
        MEMSET(obj, 0, sizeof(*obj));
    }
    if (bytes < 0) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_BSN_TLV_EGRESS_SAMPLING_RATE;
...
/**
 * Get value from an object of type of_bsn_tlv_egress_sampling_rate.
 * @param obj Pointer to an object of type of_bsn_tlv_egress_sampling_rate.
 * @param value Pointer to the child object of type
 * uint32_t to be filled out.
 *
 */
void
of_bsn_tlv_egress_sampling_rate_value_get(
    ...
    uint32_t *value)
{
    ...
    LOCI_ASSERT(obj->object_id == OF_BSN_TLV_EGRESS_SAMPLING_RATE);
    ver = obj->version;
    wbuf = OF_OBJECT_TO_WBUF(obj);
    LOCI_ASSERT(wbuf != NULL);
...
/**
 * Set value in an object of type of_bsn_tlv_egress_sampling_rate.
 * @param obj Pointer to an object of type of_bsn_tlv_egress_sampling_rate.
 * @param value The value to write into the object
 */
void
of_bsn_tlv_egress_sampling_rate_value_set(
    of_bsn_tlv_egress_sampling_rate_t *obj,
    uint32_t value)
{
    of_wire_buffer_t *wbuf;
...
void
of_bsn_tlv_encap_push_wire_types(of_object_t *obj)
{
    unsigned char *buf = OF_OBJECT_BUFFER_INDEX(obj, 0);
    switch (obj->version) {
    case OF_VERSION_1_3:
...
/**
 * \defgroup of_bsn_tlv_encap of_bsn_tlv_encap
 */

/**
 * Create a new of_bsn_tlv_encap object
 *
 * @param version The wire version to use for the object
 * @return Pointer to the newly create object or NULL on error
 *
 ...
 * a new underlying wire buffer.
 *
 * \ingroup of_bsn_tlv_encap
 */

of_object_t *
of_bsn_tlv_encap_new(of_version_t version)
{
    of_object_t *obj;
    int bytes;

    bytes = of_object_fixed_len[version][OF_BSN_TLV_ENCAP];

    if ((obj = of_object_new(bytes)) == NULL) {
        return NULL;
    }

    of_bsn_tlv_encap_init(obj, version, bytes, 0);
    of_bsn_tlv_encap_push_wire_types(obj);
    of_tlv16_wire_length_set(obj, obj->length);
...
/**
 * Initialize an object of type of_bsn_tlv_encap.
 *
 * @param obj Pointer to the object to initialize
 * @param version The wire version to use for the object
 * @param bytes How many bytes in the object
...
void
of_bsn_tlv_encap_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_BSN_TLV_ENCAP] >= 0);
    if (clean_wire) {
        MEMSET(obj, 0, sizeof(*obj));
    }
    if (bytes < 0) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_BSN_TLV_ENCAP;
...
/**
 * Get value from an object of type of_bsn_tlv_encap.
 * @param obj Pointer to an object of type of_bsn_tlv_encap.
 * @param value Pointer to the child object of type
 * uint8_t to be filled out.
 *
 */
void
of_bsn_tlv_encap_value_get(
    of_bsn_tlv_encap_t *obj,
    uint8_t *value)
{
    of_wire_buffer_t *wbuf;
    int offset = 0; /* Offset of value relative to the start obj */
    ...
    LOCI_ASSERT(obj->object_id == OF_BSN_TLV_ENCAP);
    ver = obj->version;
    wbuf = OF_OBJECT_TO_WBUF(obj);
    LOCI_ASSERT(wbuf != NULL);
...
/**
 * Set value in an object of type of_bsn_tlv_encap.
 * @param obj Pointer to an object of type of_bsn_tlv_encap.
 * @param value The value to write into the object
 */
void
of_bsn_tlv_encap_value_set(
    of_bsn_tlv_encap_t *obj,
    uint8_t value)
{
    of_wire_buffer_t *wbuf;
...
void
of_bsn_tlv_fail_count_push_wire_types(of_object_t *obj)
{
    unsigned char *buf = OF_OBJECT_BUFFER_INDEX(obj, 0);
    switch (obj->version) {
    case OF_VERSION_1_3:
    case OF_VERSION_1_4:
        *(uint16_t *)(buf + 0) = U16_HTON(0xc2); /* type */
        break;
    default:
        UNREACHABLE();
    }
...
/**
 * \defgroup of_bsn_tlv_fail_count of_bsn_tlv_fail_count
 */

/**
 * Create a new of_bsn_tlv_fail_count object
 *
 * @param version The wire version to use for the object
 * @return Pointer to the newly create object or NULL on error
 *
...
of_bsn_tlv_fail_count_new(of_version_t version)
{
    ...
    bytes = of_object_fixed_len[version][OF_BSN_TLV_FAIL_COUNT];

    if ((obj = of_object_new(bytes)) == NULL) {
        return NULL;
    }

    of_bsn_tlv_fail_count_init(obj, version, bytes, 0);
    of_bsn_tlv_fail_count_push_wire_types(obj);
    of_tlv16_wire_length_set(obj, obj->length);
...
/**
 * Initialize an object of type of_bsn_tlv_fail_count.
 *
 * @param obj Pointer to the object to initialize
 * @param version The wire version to use for the object
 * @param bytes How many bytes in the object
...
void
of_bsn_tlv_fail_count_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_BSN_TLV_FAIL_COUNT] >= 0);
    if (clean_wire) {
        MEMSET(obj, 0, sizeof(*obj));
    }
    if (bytes < 0) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_BSN_TLV_FAIL_COUNT;
...
/**
 * Get value from an object of type of_bsn_tlv_fail_count.
 * @param obj Pointer to an object of type of_bsn_tlv_fail_count.
 * @param value Pointer to the child object of type
 * uint64_t to be filled out.
 *
 */
void
of_bsn_tlv_fail_count_value_get(
    ...
    uint64_t *value)
{
    ...
    LOCI_ASSERT(obj->object_id == OF_BSN_TLV_FAIL_COUNT);
    ver = obj->version;
    wbuf = OF_OBJECT_TO_WBUF(obj);
    LOCI_ASSERT(wbuf != NULL);
...
/**
 * Set value in an object of type of_bsn_tlv_fail_count.
 * @param obj Pointer to an object of type of_bsn_tlv_fail_count.
 * @param value The value to write into the object
 */
void
of_bsn_tlv_fail_count_value_set(
    of_bsn_tlv_fail_count_t *obj,
    uint64_t value)
{
    of_wire_buffer_t *wbuf;
...
void
of_bsn_tlv_flow_classifier_push_wire_types(of_object_t *obj)
{
    unsigned char *buf = OF_OBJECT_BUFFER_INDEX(obj, 0);
    switch (obj->version) {
    case OF_VERSION_1_3:
...
/**
 * \defgroup of_bsn_tlv_flow_classifier of_bsn_tlv_flow_classifier
 */

/**
 * Create a new of_bsn_tlv_flow_classifier object
 *
 * @param version The wire version to use for the object
 * @return Pointer to the newly create object or NULL on error
 *
...
of_bsn_tlv_flow_classifier_new(of_version_t version)
{
    ...
    bytes = of_object_fixed_len[version][OF_BSN_TLV_FLOW_CLASSIFIER];

    if ((obj = of_object_new(bytes)) == NULL) {
        return NULL;
    }

    of_bsn_tlv_flow_classifier_init(obj, version, bytes, 0);
    of_bsn_tlv_flow_classifier_push_wire_types(obj);
    of_tlv16_wire_length_set(obj, obj->length);
...
/**
 * Initialize an object of type of_bsn_tlv_flow_classifier.
 *
 * @param obj Pointer to the object to initialize
 * @param version The wire version to use for the object
 * @param bytes How many bytes in the object
...
void
of_bsn_tlv_flow_classifier_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_BSN_TLV_FLOW_CLASSIFIER] >= 0);
    if (clean_wire) {
        MEMSET(obj, 0, sizeof(*obj));
    }
    if (bytes < 0) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_BSN_TLV_FLOW_CLASSIFIER;
...
/**
 * Get value from an object of type of_bsn_tlv_flow_classifier.
 * @param obj Pointer to an object of type of_bsn_tlv_flow_classifier.
 * @param value Pointer to the child object of type
 * uint16_t to be filled out.
 *
 */
void
of_bsn_tlv_flow_classifier_value_get(
    ...
    uint16_t *value)
{
    ...
    LOCI_ASSERT(obj->object_id == OF_BSN_TLV_FLOW_CLASSIFIER);
    ver = obj->version;
    wbuf = OF_OBJECT_TO_WBUF(obj);
    LOCI_ASSERT(wbuf != NULL);
...
/**
 * Set value in an object of type of_bsn_tlv_flow_classifier.
 * @param obj Pointer to an object of type of_bsn_tlv_flow_classifier.
 * @param value The value to write into the object
 */
void
of_bsn_tlv_flow_classifier_value_set(
    of_bsn_tlv_flow_classifier_t *obj,
    uint16_t value)
{
    of_wire_buffer_t *wbuf;
...
void
of_bsn_tlv_flow_classify_push_wire_types(of_object_t *obj)
{
    unsigned char *buf = OF_OBJECT_BUFFER_INDEX(obj, 0);
    switch (obj->version) {
    case OF_VERSION_1_3:
...
/**
 * \defgroup of_bsn_tlv_flow_classify of_bsn_tlv_flow_classify
 */

/**
 * Create a new of_bsn_tlv_flow_classify object
 *
 * @param version The wire version to use for the object
 * @return Pointer to the newly create object or NULL on error
 *
...
of_bsn_tlv_flow_classify_new(of_version_t version)
{
    ...
    bytes = of_object_fixed_len[version][OF_BSN_TLV_FLOW_CLASSIFY];

    if ((obj = of_object_new(bytes)) == NULL) {
        return NULL;
    }

    of_bsn_tlv_flow_classify_init(obj, version, bytes, 0);
    of_bsn_tlv_flow_classify_push_wire_types(obj);
    of_tlv16_wire_length_set(obj, obj->length);
...
/**
 * Initialize an object of type of_bsn_tlv_flow_classify.
 *
 * @param obj Pointer to the object to initialize
 * @param version The wire version to use for the object
 * @param bytes How many bytes in the object
...
void
of_bsn_tlv_flow_classify_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_BSN_TLV_FLOW_CLASSIFY] >= 0);
    if (clean_wire) {
        MEMSET(obj, 0, sizeof(*obj));
    }
    if (bytes < 0) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_BSN_TLV_FLOW_CLASSIFY;
