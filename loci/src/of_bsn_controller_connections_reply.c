    ...
    of_version_t version, int bytes, int clean_wire)
{
    ...
    ASSERT(of_object_fixed_len[version][OF_BSN_CONTROLLER_CONNECTIONS_REPLY] >= 0);
    if (clean_wire) {
    ...
    uint32_t *xid)
{
    ...
    ASSERT(obj->object_id == OF_BSN_CONTROLLER_CONNECTIONS_REPLY);
    ver = obj->version;
    ...
    of_list_bsn_controller_connection_t *connections)
{
    ...
    if (obj->wire_object.wbuf == connections->wire_object.wbuf) {
        ...
        ASSERT(abs_offset == OF_OBJECT_ABSOLUTE_OFFSET(connections, 0));
