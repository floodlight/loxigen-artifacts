...
void
of_bsn_tlv_digest_alg_push_wire_types(of_object_t *obj)
{
    unsigned char *buf = OF_OBJECT_BUFFER_INDEX(obj, 0);
    switch (obj->version) {
    case OF_VERSION_1_3:
    case OF_VERSION_1_4:
        *(uint16_t *)(buf + 0) = U16_HTON(0xa2); /* type */
        break;
    default:
        UNREACHABLE();
    }
...
/**
 * \defgroup of_bsn_tlv_digest_alg of_bsn_tlv_digest_alg
 */

/**
 * Create a new of_bsn_tlv_digest_alg object
 *
 * @param version The wire version to use for the object
 * @return Pointer to the newly create object or NULL on error
 *
 ...
 * a new underlying wire buffer.
 *
 * \ingroup of_bsn_tlv_digest_alg
 */

of_object_t *
of_bsn_tlv_digest_alg_new(of_version_t version)
{
    of_object_t *obj;
    int bytes;

    bytes = of_object_fixed_len[version][OF_BSN_TLV_DIGEST_ALG];

    if ((obj = of_object_new(bytes)) == NULL) {
        return NULL;
    }

    of_bsn_tlv_digest_alg_init(obj, version, bytes, 0);
    of_bsn_tlv_digest_alg_push_wire_types(obj);
    of_tlv16_wire_length_set(obj, obj->length);
...
/**
 * Initialize an object of type of_bsn_tlv_digest_alg.
 *
 * @param obj Pointer to the object to initialize
 * @param version The wire version to use for the object
 * @param bytes How many bytes in the object
...
void
of_bsn_tlv_digest_alg_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_BSN_TLV_DIGEST_ALG] >= 0);
    if (clean_wire) {
        MEMSET(obj, 0, sizeof(*obj));
    }
    if (bytes < 0) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_BSN_TLV_DIGEST_ALG;
...
/**
 * Get value from an object of type of_bsn_tlv_digest_alg.
 * @param obj Pointer to an object of type of_bsn_tlv_digest_alg.
 * @param value Pointer to the child object of type
 * uint8_t to be filled out.
 *
 */
void
of_bsn_tlv_digest_alg_value_get(
    of_bsn_tlv_digest_alg_t *obj,
    uint8_t *value)
{
    of_wire_buffer_t *wbuf;
    int offset = 0; /* Offset of value relative to the start obj */
    ...
    LOCI_ASSERT(obj->object_id == OF_BSN_TLV_DIGEST_ALG);
    ver = obj->version;
    wbuf = OF_OBJECT_TO_WBUF(obj);
    LOCI_ASSERT(wbuf != NULL);
...
/**
 * Set value in an object of type of_bsn_tlv_digest_alg.
 * @param obj Pointer to an object of type of_bsn_tlv_digest_alg.
 * @param value The value to write into the object
 */
void
of_bsn_tlv_digest_alg_value_set(
    of_bsn_tlv_digest_alg_t *obj,
    uint8_t value)
{
    of_wire_buffer_t *wbuf;
