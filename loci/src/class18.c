...
void
of_bsn_tlv_redundant_mgmt_reselect_push_wire_types(of_object_t *obj)
{
    unsigned char *buf = OF_OBJECT_BUFFER_INDEX(obj, 0);
    switch (obj->version) {
    case OF_VERSION_1_3:
...
/**
 * \defgroup of_bsn_tlv_redundant_mgmt_reselect of_bsn_tlv_redundant_mgmt_reselect
 */

/**
 * Create a new of_bsn_tlv_redundant_mgmt_reselect object
 *
 * @param version The wire version to use for the object
 * @return Pointer to the newly create object or NULL on error
 *
...
of_bsn_tlv_redundant_mgmt_reselect_new(of_version_t version)
{
    ...
    bytes = of_object_fixed_len[version][OF_BSN_TLV_REDUNDANT_MGMT_RESELECT];

    if ((obj = of_object_new(bytes)) == NULL) {
        return NULL;
    }

    of_bsn_tlv_redundant_mgmt_reselect_init(obj, version, bytes, 0);
    of_bsn_tlv_redundant_mgmt_reselect_push_wire_types(obj);
    of_tlv16_wire_length_set(obj, obj->length);
...
/**
 * Initialize an object of type of_bsn_tlv_redundant_mgmt_reselect.
 *
 * @param obj Pointer to the object to initialize
 * @param version The wire version to use for the object
 * @param bytes How many bytes in the object
...
void
of_bsn_tlv_redundant_mgmt_reselect_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_BSN_TLV_REDUNDANT_MGMT_RESELECT] >= 0);
    if (clean_wire) {
        MEMSET(obj, 0, sizeof(*obj));
    }
    if (bytes < 0) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_BSN_TLV_REDUNDANT_MGMT_RESELECT;
...
void
of_bsn_tlv_udf_data_push_wire_types(of_object_t *obj)
{
    unsigned char *buf = OF_OBJECT_BUFFER_INDEX(obj, 0);
    switch (obj->version) {
    case OF_VERSION_1_3:
    case OF_VERSION_1_4:
        *(uint16_t *)(buf + 0) = U16_HTON(0xcf); /* type */
        break;
    default:
        UNREACHABLE();
    }
...
/**
 * \defgroup of_bsn_tlv_udf_data of_bsn_tlv_udf_data
 */

/**
 * Create a new of_bsn_tlv_udf_data object
 *
 * @param version The wire version to use for the object
 * @return Pointer to the newly create object or NULL on error
 *
...
of_bsn_tlv_udf_data_new(of_version_t version)
{
    ...
    bytes = of_object_fixed_len[version][OF_BSN_TLV_UDF_DATA];

    if ((obj = of_object_new(bytes)) == NULL) {
        return NULL;
    }

    of_bsn_tlv_udf_data_init(obj, version, bytes, 0);
    of_bsn_tlv_udf_data_push_wire_types(obj);
    of_tlv16_wire_length_set(obj, obj->length);
...
/**
 * Initialize an object of type of_bsn_tlv_udf_data.
 *
 * @param obj Pointer to the object to initialize
 * @param version The wire version to use for the object
 * @param bytes How many bytes in the object
...
void
of_bsn_tlv_udf_data_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_BSN_TLV_UDF_DATA] >= 0);
    if (clean_wire) {
        MEMSET(obj, 0, sizeof(*obj));
    }
    if (bytes < 0) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_BSN_TLV_UDF_DATA;
...
/**
 * Get value from an object of type of_bsn_tlv_udf_data.
 * @param obj Pointer to an object of type of_bsn_tlv_udf_data.
 * @param value Pointer to the child object of type
 * uint32_t to be filled out.
 *
 */
void
of_bsn_tlv_udf_data_value_get(
    ...
    uint32_t *value)
{
    ...
    LOCI_ASSERT(obj->object_id == OF_BSN_TLV_UDF_DATA);
    ver = obj->version;
    wbuf = OF_OBJECT_TO_WBUF(obj);
    LOCI_ASSERT(wbuf != NULL);
...
/**
 * Set value in an object of type of_bsn_tlv_udf_data.
 * @param obj Pointer to an object of type of_bsn_tlv_udf_data.
 * @param value The value to write into the object
 */
void
of_bsn_tlv_udf_data_value_set(
    of_bsn_tlv_udf_data_t *obj,
    uint32_t value)
{
    of_wire_buffer_t *wbuf;
