...
public class OFFlowDescReplyVer15Test {
    OFFactory factory;

    final static byte[] FLOW_DESC_REPLY_SERIALIZED =
    ...
    public void testWrite() {
        OFFlowDescReply.Builder builder = factory.buildFlowDescReply();
        builder
        .setXid(0x12345678)
        .setFlags(ImmutableSet.<OFStatsReplyFlags>of(OFStatsReplyFlags.REPLY_MORE))
        .setEntries(ImmutableList.<OFFlowDescEntry>of(
                        factory.buildFlowDescEntry().setTableId(TableId.of(1))
                                                    .setPriority(1)
                                                    .setIdleTimeout(16)
                                                    .setHardTimeout(96)
                                                    ...
                                                    .build() ))
        .build();
        OFFlowDescReply flowDescReply = builder.build();
        ByteBuf bb = Unpooled.buffer();
        flowDescReply.writeTo(bb);
        byte[] written = new byte[bb.readableBytes()];
        bb.readBytes(written);

        assertThat(written, CoreMatchers.equalTo(FLOW_DESC_REPLY_SERIALIZED));
    }
    ...
    public void testRead() throws Exception {
        ...
        .setEntries(ImmutableList.<OFFlowDescEntry>of(
                        factory.buildFlowDescEntry().setTableId(TableId.of(1))
                                                    ...
                                                    .build() ))
        .build();
        OFFlowDescReply flowDescReplyBuilt = builder.build();

        ByteBuf input = Unpooled.copiedBuffer(FLOW_DESC_REPLY_SERIALIZED);

        // FIXME should invoke the overall reader once implemented
        OFFlowDescReply flowDescReplyRead = OFFlowDescReplyVer15.READER.readFrom(input);
        assertEquals(FLOW_DESC_REPLY_SERIALIZED.length, input.readerIndex());

        assertEquals(flowDescReplyBuilt, flowDescReplyRead);
   }
   ...
   public void testReadWrite() throws Exception {
       ...
       // FIXME should invoke the overall reader once implemented
       OFFlowDescReply flowDescReply = OFFlowDescReplyVer15.READER.readFrom(input);
       assertEquals(FLOW_DESC_REPLY_SERIALIZED.length, input.readerIndex());
