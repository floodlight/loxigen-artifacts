 ...
 *
 * <h3>(1) Ordering:</h3>
 * Be careful about the serialization order of child nodes when using collections, e.g., a List or a Set.
 * If the order does not matter for the logical equality (i.e., Set semantics), make sure you dump
 * the child objects in stable,  deterministic order. If the data comes from a data which does
 * not guarantee deterministic, stable iteration order (note that most don't: e.g., {@link HashSet},
 * which does not guarantee iteration order, or {@link LinkedHashSet} or Guava's {@link ImmutableSet}
 * which yield objects in insertion order), make sure to sort the entries before dumping them, or
 * use a {@link SortedSet}.
 *
 * <pre>
 * {@code
 *
