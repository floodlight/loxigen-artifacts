...
class bsn_ifp_class_id(oxm):
    ...
    @staticmethod
    def unpack(reader):
        obj = bsn_ifp_class_id()
        _type_len = reader.read("!L")[0]
        assert(_type_len == 210436)
        obj.value = reader.read("!L")[0]
        return obj
...
oxm.subtypes[210436] = bsn_ifp_class_id

class bsn_ifp_class_id_masked(oxm):
    type_len = 210696
    ...
    @staticmethod
    def unpack(reader):
        obj = bsn_ifp_class_id_masked()
        _type_len = reader.read("!L")[0]
        assert(_type_len == 210696)
        obj.value = reader.read("!L")[0]
        obj.value_mask = reader.read("!L")[0]
        return obj
    ...
    def pretty_print(self, q):
        q.text("bsn_ifp_class_id_masked {")
        with q.group():
            with q.indent(2):
                q.breakable()
                q.text("value = ");
...
oxm.subtypes[210696] = bsn_ifp_class_id_masked
